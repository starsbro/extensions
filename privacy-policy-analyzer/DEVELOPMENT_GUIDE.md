...
try {
  const { PrivacyPolicyAnalyzer } = await import(
    chrome.runtime.getURL("scripts/content-main.js")
  );
} catch (error) {
...
const patterns = [
  {
    ...
    pattern:
      /(share|sell|transfer).{0,50}(personal|data).{0,50}(third.party)/gi,
    severity: "high",
             ...
             messages: [
               {
                 role: "system",
                 content:
                   "Analyze this privacy policy for concerning clauses...",
               },
               {
                 role: "user",
                 content: textContent,
